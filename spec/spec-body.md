...
Deriving the blinding salt per copy is what bounds the reach of that salt, and is the reason the derivation is at `k.s` rather than at a path shared by the set. The blinding salt is held by the Issuer and by the designated Discloser and is not disclosed to a Disclosee, as the disclosure procedure below requires, so what a shared salt widens is not a Disclosee's reach but the blast radius of any sharing or compromise of the salt itself. A salt shared across the set would permit its holder to unblind every state update of every member of the set, reintroducing as a correlator precisely what independent Registries remove. The hazard is sharper than it may appear, because the derivation path for a blind within a Registry is the sequence number of the transaction event alone, so two Registries that share a blinding salt do not merely admit unblinding by a common holder of that salt but produce the identical blind at the same sequence number, which correlates the two Registries to any observer comparing their events, with no salt at hand at all. An Issuer may nonetheless have a countervailing reason to share one blinding salt across a group of Registries, such as enabling a Disclosee to validate a chain of authority whose intermediate blinded states it has no other means to obtain. That is outside independent Registry bulk issuance, whose whole purpose is the decorrelation such sharing gives up, and an Issuer that shares a blinding salt across a group accepts provable correlation within that group.
...
3. The applicable state update event from that Registry, together with the blind of that event when it is a blindable state update, `bup`, event. A Discloser MUST NOT disclose the blinding salt of copy `k`.
...
3. Unblinding the state update event, when it is blinded, using the disclosed blind, and confirming that the resulting transaction event ACDC SAID, `td`, field value is d<sub>k</sub>.
...
Item 3 of the disclosure, and step 3 of the verification, turn on the blind of one event rather than on the blinding salt from which that blind derives, and the distinction is load bearing. The blind of one event permits a Disclosee to unblind that event and no other. The blinding salt permits it to unblind every event of that Registry at every sequence number, those already published and those not yet written, because the derivation path for a blind within a Registry is the sequence number of the transaction event alone and the sequence number, `n`, field value is disclosed in the clear by every `bup` event. A Disclosee given the salt therefore holds a standing, non-interactive read on the state of copy `k`, including its eventual revocation, and the remedy of the Blinded State Disclosure section above, by which the Discloser has the Issuer publish a further update under a new blinding factor, has no effect on that Disclosee, because the new blinding factor derives from the salt it already holds. A Disclosee that requires the current state MUST therefore obtain a fresh blind from the Discloser for each state update it wishes to read.
